...
export default function useGameNFT(contractAddress: Address | undefined, tokenIds: number[]) {
    ...
    ;(async () => {
      try {
        ...
        for (const id of tokenIds) {
          // ownerOf
          try {
            const owner = await publicClient.readContract({ address: contractAddress, abi: abi as any, functionName: 'ownerOf', args: [BigInt(id)] })
            o[id] = String(owner)
          } catch (e) {
            // ignore
          }
          ...
          try {
            const uriRaw = await publicClient.readContract({ address: contractAddress, abi: abi as any, functionName: 'tokenURI', args: [BigInt(id)] })
            ...
            if (resolvedName) nms[id] = resolvedName
          } catch (e) {
